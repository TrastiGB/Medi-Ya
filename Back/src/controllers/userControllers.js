...
const createUser = async (req, res) => {
  try {
    const { nombre, email, password, tipo = "administrador" } = req.body;
    ...
    await User.create({ nombre, email, password: hashedPassword, tipo });
...
const updateUser = async (req, res) => {
  try {
    ...
    if (password) {
      const hashedPassword = await bcrypt.hash(password, 10);
      await user.update({ nombre, email, tipo, password: hashedPassword });
    } else {
